...
for r in R_values:
    ds = xr.open_dataset(f"data/lorenz_data_r={r}.nc")

    x = ds["state"].sel(dim="x").values
    y = ds["state"].sel(dim="y").values
    z = ds["state"].sel(dim="z").values

    ax.plot(x, y, z, lw=0.5, label=f"r={r}")
